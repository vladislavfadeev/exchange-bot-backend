...
class ResponseViewDataActions:
    def all_responses(request, *args, **kwargs):
        ...
        queryset = ResponseModel.objects.filter(changer_id_id=pk)
        return queryset
    ...
    def create_action(request):
        ...
        request_info = RequestModel.objects.get(pk=request.data["request_id"])
...
class ChoiseViewDataActions:
    def create_action(request):
        choice_data = ResponseModel.objects.get(pk=request.data["response_id"])
...
class TransactionViewDataActions:
    def create_action(request):
        transaction_data = CustomerChoice.objects.get(pk=request.data["post_choice_id"])
        data = {
            "post_choice_id": transaction_data.id,
            "customer_bank": transaction_data.response_id.customer_bank,
            "changer_bank": transaction_data.response_id.changer_bank,
