...
class AllMongolianBanks(models.Model):
    name = models.CharField("Название банка", max_length=50, unique=True)
...
class ChangerBankAccount(models.Model):
    ...
    bankAccount = models.BigIntegerField("Номер счета", unique=True)
    ...
    currency = models.ForeignKey(
        Currency, verbose_name="Валюта", on_delete=models.DO_NOTHING
    )
...
class ChangerOffer(models.Model):
    ...
    owner = models.ForeignKey(
        Changer, verbose_name="Владелец", on_delete=models.CASCADE
    )
    bannerName = models.CharField("Название оффера", max_length=64, blank=True)
    ...
    minAmount = models.FloatField("Минимальная сумма сделки", blank=True, null=True)
    maxAmount = models.FloatField("Макимальная сумма сделки", blank=True, null=True)
    dateCreated = models.DateTimeField("Дата создания", auto_now_add=True)
...
class ChangerScore(models.Model):
    owner = models.OneToOneField(
        Changer, related_name="score", on_delete=models.CASCADE, primary_key=True
    )
    ...
    def avg_time(self):
        total_time = 0
        queryset = Transaction.objects.filter(changer=self.owner)
        for i in queryset:
...
class Transaction(models.Model):
    changer = models.ForeignKey(Changer, on_delete=models.CASCADE)
    offer = models.ForeignKey(ChangerOffer, on_delete=models.CASCADE)
    user = models.ForeignKey(BotUser, on_delete=models.CASCADE)
    changerBank = models.ForeignKey(ChangerBankAccount, on_delete=models.CASCADE)
    userBank = models.ForeignKey(UserBankAccount, on_delete=models.CASCADE)
    ...
    userSendMoneyDate = models.DateTimeField(blank=True, null=True)
    userAcceptDate = models.DateTimeField(blank=True, null=True)
    changerSendMoneyDate = models.DateTimeField(blank=True, null=True)
    changerAcceptDate = models.DateTimeField(blank=True, null=True)
    userProofType = models.CharField(max_length=50, blank=True)
    changerProofType = models.CharField(max_length=50, blank=True)
    userProof = models.CharField(max_length=500, blank=True)
    changerProof = models.CharField(max_length=500, blank=True)
